...
import {PlusIcon, UploadCloud, FilePen, Trash2, Pencil, XIcon, LoaderCircleIcon} from 'lucide-react'
    ...
    <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
      {allResumes.map((resume, index)=>{
        ...
        return (
          ...
          {{background: `linear-gradient(135deg, ${baseColor}10 0%, ${baseColor}40 100%)`, borderColor: baseColor + '40'}}>
            
           <FilePen className="size-7 group-hover:scale-105
           ...
           hidden'>
            <Trash2 onClick={()=>deleteResume(resume._id)} className="size-7 p-1.5 hover:bg-white/50 rounded-2xl
            text-slate-700 transition-colors"/>

            <Pencil onClick={()=> {setEditResumeId(resume._id); setTitle
            (resume.title)}} className="size-7 p-1.5 hover:bg-white/50 rounded-2xl
            text-slate-700 transition-colors"/>
